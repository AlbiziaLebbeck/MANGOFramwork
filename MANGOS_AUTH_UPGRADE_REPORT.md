...
## Avatar Thumbnail, Profile Image, and Rig Correction Follow-up

The avatar selector created an icon for every URL but no longer started a loader for uncached account avatars. Consequently, only an avatar that had already loaded as the current player could generate an icon texture. The selector now loads each unique default and account avatar, reuses cached models when available, and generates a thumbnail as soon as that model is ready. Preview models are positioned outside the gameplay camera range and removed after capture so a malformed preview cannot appear in the world.

Thumbnail framing no longer depends on finding a child whose name contains `Head`. That heuristic selected inconsistent transforms across Avaturn, VRoid, Ready Player Me, and legacy rigs. A temporary isolated orthographic camera now frames the complete renderer bounds with padding, captures only the target avatar, restores all temporary render layers, restores the previous active render texture, and releases temporary camera/render-texture resources. Generated textures update the persistent user image when they correspond to the account default avatar.

The documented `users/me` field `avatarImageUrl` is now resolved against the configured API origin and loaded as a best-effort preferred account profile image. If the image is absent, protected, fails CORS, times out, or cannot be decoded, the generated default-avatar thumbnail remains the fallback. A preferred account profile image is not overwritten by a later thumbnail completion.

`RajPattern_Blue.glb` was downloaded from its public production URL and its actual node hierarchy was inspected. It contains both the specific `Armature/Outfit` marker and the generic `Armature/mixamorig:Hips` marker. The previous detection order selected `full_derssAvatar` before reaching `RajPatternAvatar`, which explains the regression. Specific RajPattern detection now precedes generic Mixamo detection.

The reported VRoid asset was also inspected as GLB/VRM data. Its Humanoid eye nodes are `J_Adj_L_FaceEye` and `J_Adj_R_FaceEye`. The VRoid path now attaches a narrowly scoped late-pose stabilizer that preserves the imported local rest position and rotation of only those two eye bones after Humanoid animation evaluation. Other avatar families are not modified.

The WebGL build revision is updated to `20260723-avatar-ui-rigs-v2`. The full Unity-generated C# solution compiles with zero errors; existing dependency version and serialized DTO warnings remain. Static checks confirm that RajPattern-specific detection precedes generic Mixamo detection and that both VRoid eye node names match the downloaded asset. Interactive thumbnail rendering, profile-image CORS behavior, RajPattern locomotion, and VRoid eye appearance still require verification in a freshly rebuilt and deployed WebGL client because the project is open in other Unity Editor processes.
